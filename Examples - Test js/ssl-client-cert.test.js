...
describe('SSL Client certificates', function () {
    ...
    it('should work correctly with standalone client certificates', function (done) {
        ...
        exec('node start.js run test/fixtures/run/ssl-client-cert.json --ssl-client-cert test/fixtures/ssl/client.crt --ssl-client-key test/fixtures/ssl/client.key --ssl-client-passphrase password -k', function (code) {
    ...
    it('should work correctly with a trusted CA certificate provided', function (done) {
        // eslint-disable-next-line max-len
        exec('node start.js run test/fixtures/run/ssl-client-cert.json --ssl-client-cert test/fixtures/ssl/client.crt --ssl-client-key test/fixtures/ssl/client.key --ssl-client-passphrase password --ssl-extra-ca-certs test/fixtures/ssl/ca.crt', function (code) {
            expect(code, 'should have exit code of 0').to.equal(0);
            done();
        });
    });
    ...
    it('should work correctly with multiple client certificates', function (done) {
        // eslint-disable-next-line max-len
        exec('node start.js run test/fixtures/run/ssl-client-cert-list.json --verbose --ssl-client-cert-list ./test/fixtures/files/ssl-client-cert-config.json -k', function (code) {
            expect(code, 'should have exit code of 0').to.equal(0);
            done();
        });
    });
    ...
    it('should use certificate from list when both client certificates options are used', function (done) {
        var cmd = 'node start.js run test/fixtures/run/ssl-client-cert-list.json' +
    ...
    it('should fallback to individual client cert when no cert from list match', function (done) {
        var cmd = 'node start.js run test/fixtures/run/ssl-client-cert.json' +
            ' --ssl-client-cert-list test/fixtures/files/ssl-client-cert-config.json' +
            ' --ssl-client-cert test/fixtures/ssl/client.crt' +
            ' --ssl-client-key test/fixtures/ssl/client.key' +
            ' --ssl-client-passphrase password -k';
